...
public class StatPrinter {
    ...
    {
	int max = max(data);
	_frequency = new ArrayList<Integer>(); //have to initialize it too
	int ctr; //finds frequency of each number
	for (int num = 0; num <= max + 1; num++){ //go through each
	    //number up to the maximum
	    ctr = 0; //reset the freqency counter
	    for (int i = 0; i < data.size(); i++) //traverse the arraylist
		if (data.get(i) == num) //check if you've found the number at the index
		    ctr += 1; //if so, add 1
	    _frequency.add(ctr); //add the frequency to the alist
	}
    }
    ...
    public Integer max( ArrayList <Integer> data ) //O(n) time 
    { 
	Integer maximum = 0; //what it'll return
	for (int i = 0; i < data.size(); i++) //traverse the alist
		if (data.get(i) > maximum) //if you've found something
		    //bigger than the reigning champ
		    maximum = data.get(i); //it's the new max
	return maximum; 
    }
    ...
    //    isLocalMode(1) -> true
    //    isLocalMode(5) -> true
    public boolean isLocalMode( int i ) //O(1) time
    { 
	return (i != 0 && //not at the beginning
		i != _frequency.size() && //not at the end
		_frequency.get(i) > _frequency.get(i + 1) && //greater than left neighbour
		_frequency.get(i) > _frequency.get(i - 1)); //greater than right neighbor
    }


    //*************** QUESTION 04 **************************
    //postcond: returns list of modes in _frequency
    public ArrayList<Integer> getLocalModes() //O(n) time
    {
	ArrayList<Integer> retAL = new ArrayList<Integer>();//arraylist to return
	for(int i = 0; i < _frequency.size() - 1; i++) //traverse freq al
		if(isLocalMode(i)) //if it's a local mode
		    retAL.add(i); //add it
	return retAL; //return the list full of local modes
    }


    //*************** QUESTION 05 **************************
    //precond:  longestBar > 0
    public void printHistogram( int longestBar ) //O(n^2) time
    {
	int max = max(_frequency);
	for (int i = 0; i < _frequency.size(); i++){ //go to each number
	    System.out.print(i + " : "); //print the number and a colon
	    for (int strs = 0; // number of stars
		 strs < longestBar * _frequency.get(i) / max;
		 //we make the bar as long as the frequency times the
		 //conversion factor. The conversion factor is the length
		 //of the longest bar divided by the highest frequency
		 strs++)
		System.out.print("*"); //we're not printing lines,
	    //just continue on the same line
	    System.out.print("\n");
	}
    }
